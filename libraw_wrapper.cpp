...
// -----------------------------------------------------------------------
// X. Black level API (真正可用的版本)
// -----------------------------------------------------------------------
    val getBlackLevels() {
        if (!processor_) return val::undefined();


        auto& C = processor_->imgdata.color;
        val result = val::object();

        // 1. 标准 cblack
        val cblackArr = val::array();
        for (int i = 0; i < 4; i++) cblackArr.set(i, C.cblack[i]);
        result.set("cblack", cblackArr);

        // 2. 全局 black
        result.set("black", C.black);

        // 3. DNG 专用字段
        val dngArr = val::array();
        for (int i = 0; i < 4; i++) dngArr.set(i, C.dng_levels.dng_cblack[i]);
        result.set("dng_cblack", dngArr);
        result.set("dng_black", C.dng_levels.dng_black);

        // 4. 白电平参考
        result.set("maximum", C.maximum);

        return result;
    }
// 获取绝对原始的 Buffer 数据 (对齐 rawpy.raw_image)
    val getRawImage() {
        if (!processor_) return val::undefined();
        ...
        // 注意：这里我们故意 【不调用】 dcraw_process()
        // 这样才能拿到包含黑边、未减黑电平的数据

        // 2. 获取原始数据指针
        // raw_image 是 LibRaw 存储原始 Bayer 数据的数组
        uint16_t* rawData = processor_->imgdata.rawdata.raw_image;
        
        if (!rawData) {
             throw std::runtime_error("raw_image is null (maybe formatting not supported?)");
        }

        // 3. 获取包含黑边的完整尺寸 (raw_width x raw_height)
        // 你的 Python 结果是 4008x6008，对应的就是这两个值
        unsigned width = processor_->imgdata.sizes.raw_width;
        unsigned height = processor_->imgdata.sizes.raw_height;
        size_t count = width * height;

        // 4. 拷贝数据回 JS
        val typedArrayCtor = val::global("Uint16Array");
        val typedArray = typedArrayCtor.new_(val(count));
        val memView = val(typed_memory_view(count, rawData));
        typedArray.call<void>("set", memView);

        // 5. 返回对象
        val res = val::object();
        res.set("data", typedArray);
        res.set("width", width);
	...
	val metadata(bool fullOutput=false) {
		...
		if(fullOutput) {
			...
			{
  			  val cblackArr = val::array();
  			  // imgdata.color.cblack 是 LibRaw 核心最原始的黑电平数据 (4通道)
  			  // rawpy 的 black_level_per_channel 就是直接读的这个
  			  for (int i = 0; i < 4; i++) {
  			      cblackArr.set(i, processor_->imgdata.color.cblack[i]);
  			  }
  			  colorData.set("cblack_rawpy_style", cblackArr); 
			}
			// 在 metadata 函数的 colorData 部分加入：

			// 1. 复刻 rawpy 的核心来源：cblack (通用 4 通道)
			{
				val cblackArr = val::array();
				// imgdata.color.cblack 是 LibRaw 汇总后的最终黑电平
				// 无论是 Canon, Nikon 还是 Sony，LibRaw 都会把值填到这里
				for (int i = 0; i < 4; i++) {
					cblackArr.set(i, processor_->imgdata.color.cblack[i]);
				}
				colorData.set("cblack_rawpy_style", cblackArr); 
			...
			// 2. 复刻 DNG 专用来源：dng_levels (针对 DNG/手机 Raw)
			// 很多时候 DNG 的 cblack 是 0，真实值藏在这里
			{
				val dngObj = val::object();
				val dngBlackArr = val::array();
				
				for (int i = 0; i < 4; i++) {
					dngBlackArr.set(i, processor_->imgdata.color.dng_levels.dng_cblack[i]);
				}
				
				dngObj.set("dng_cblack", dngBlackArr);
				colorData.set("dng_levels", dngObj);
			}
			{
				val dngObj = val::object();
				val dngBlackArr = val::array();
				val dngWhiteArr = val::array();

				// dng_levels.dng_cblack 是 DNG 专用的 4通道黑电平
				for (int i = 0; i < 4; i++) {
					dngBlackArr.set(i, processor_->imgdata.color.dng_levels.dng_cblack[i]);
					dngWhiteArr.set(i, processor_->imgdata.color.dng_levels.dng_whitelevel[i]);
				}
				
				dngObj.set("dng_cblack", dngBlackArr);       // DNG 黑电平
				dngObj.set("dng_whitelevel", dngWhiteArr);   // DNG 白电平
				
				// 同时也把模拟黑电平暴露出来 (有些 DNG 用这个)
				dngObj.set("dng_black", processor_->imgdata.color.dng_levels.dng_black);
...
EMSCRIPTEN_BINDINGS(libraw_module) {
    ...
    class_<WASMLibRaw>("LibRaw")
        ...
        .function("getRawImage",    &WASMLibRaw::getRawImage)    // God Mode
        .function("getBlackLevels", &WASMLibRaw::getBlackLevels) // Detective Mode
